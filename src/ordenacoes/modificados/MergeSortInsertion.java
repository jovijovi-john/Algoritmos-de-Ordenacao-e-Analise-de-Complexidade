...
import main.Item;
import ordenacoes.InsertionSort;

public class MergeSortInsertion {
    ...
    public static void mergeSort(Item<?, ?>[] vetor, Item<?, ?>[] vetor_aux, int inicio, int fim, int ordem){
        if (fim - inicio < 15) {
            long [] comparacoesAtribuicoes = InsertionSort.sort(vetor, ordem);
            comparacoes += comparacoesAtribuicoes[0];
            atribuicoes += comparacoesAtribuicoes[1];
        } else {
            if (inicio < fim) {
